...
#'
#' @param df
#' @param haplos Data frame, contains haplotype assignments for each CC strain as generated by haploscores function (haploscores, haploscores2, haploscores2, or quickscores).
...
addhap <- function(df,haplos,locus.name,strain.col) {
  ...
  if(any("RIX" %in% colnames(df)) && any(!("dam" %in% colnames(df)))) {
    ...
    df <- add.stat.base(df,haplos,locus.name)
      ...
      colnames(haplos)[1] <- strain.col
      haplos=haplos[, names(haplos) != "alias"]
      df <- merge(df,haplos)
...
addmx1 <- function(df,strain.col,haplos,locus.name,cast.score=0.5){
  if (cast.score==1) cast.col=9
  if (cast.score==0.5) cast.col=10
  if (cast.score==0) cast.col=11
  if (missing(cast.score)) cast.col=10

  mx1.score <- mx1[c(1,cast.col)]
  colnames(mx1.score)[2] <- "allele.effects"

  df <- addhap(df,haplos=mx1.score,locus.name="mx1",strain.col)
  return(df)
}
